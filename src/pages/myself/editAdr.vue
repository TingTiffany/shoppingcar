<template>
	<div class="adr-cont">
		<xheader htitle="编辑收货地址" :leftOpt="leftOpt"></xheader>
...
<script>
	...
	export default {
		...
		created(){
			this.adrObj = this.adrArr[this.adrId];
		},
		methods: {
			getAdrName(val){
			...
			saveAdr(){
				if(this.adrObj.nam == ''){
					...
					this.adrObj.adrf = this.getAdrName(this.adrObj.adr).replace(/ /g, '') + this.adrObj.adrs;
					if(this.adrObj.is){
						let id = this.adrId;
						this.adrArr.forEach(function(obj, i){
							if(i != id && obj.is){
								obj.is = false;
							}
						});
